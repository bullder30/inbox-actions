...
import { IMAP_PRESETS, detectProviderFromEmail, isMicrosoftProvider } from "@/lib/imap/types";
...
const formSchema = z.object({
  ...
  authMethod: z.enum(["PASSWORD", "OAUTH"]).default("PASSWORD"),
}).refine((data) => {
  // Password is required only for PASSWORD auth method
  if (data.authMethod === "PASSWORD" && (!data.imapPassword || data.imapPassword.length === 0)) {
    return false;
  }
  return true;
}, {
  message: "Mot de passe requis pour l'authentification par mot de passe",
  path: ["imapPassword"],
});
...
export function IMAPConnectForm({ onSuccess }: IMAPConnectFormProps) {
  ...
  const handleEmailChange = (email: string) => {
    ...
    // Check if Microsoft email - should use Graph API
    if (isMicrosoftProvider(detectedProvider)) {
      ...
      <CardContent>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Microsoft email detected - redirect to Graph API */}
            {isMicrosoftEmail && (
              <Alert className="border-purple-200 bg-purple-50 dark:border-purple-800 dark:bg-purple-950">
                ...
                <AlertTitle className="text-purple-800 dark:text-purple-200">
                  Compte Microsoft détecté
                </AlertTitle>
                <AlertDescription className="mt-2 space-y-3 text-purple-700 dark:text-purple-300">
                  <p>
                    Pour les comptes Microsoft (Outlook, Hotmail, Live), nous recommandons
                    d&apos;utiliser <strong>Microsoft Graph API</strong> au lieu d&apos;IMAP.
                  </p>
                  <p className="text-sm">
                    Graph API offre une meilleure expérience : pas de configuration manuelle,
                    synchronisation plus rapide, et aucun mot de passe requis.
                  </p>
                  <p className="text-sm font-medium">
                    Connectez-vous avec Microsoft depuis la page de connexion pour activer Graph API.
                  </p>
                </AlertDescription>
              </Alert>
            )}

            {/* Password - with provider-specific description (hidden for OAuth) */}
            {authMethod === "PASSWORD" && (
              <FormField
                control={form.control}
                name="imapPassword"
                render={({ field }) => (
                  <FormItem>
                    <FormLabel>
                      {providerHelp?.passwordLabel || "Mot de passe d'application"}
                    </FormLabel>
                    <FormControl>
                      <div className="relative">
