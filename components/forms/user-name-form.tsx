...
export function UserNameForm({ user }: UserNameFormProps) {
    ...
    startTransition(async () => {
      ...
      if (status !== "success") {
        toast.error("Une erreur s'est produite.", {
          description: "Votre nom n'a pas été mis à jour. Veuillez réessayer.",
        });
      } else {
        await update();
        setUpdated(false);
  ...
  return (
    <form onSubmit={onSubmit}>
      <SectionColumns
        title="Votre nom"
        description="Entrez un nom d'affichage avec lequel vous êtes à l'aise."
      >
        ...
        <div className="flex flex-col justify-between p-1">
          ...
          <p className="text-[13px] text-muted-foreground">Maximum 32 caractères</p>
        </div>
      </SectionColumns>
    </form>
  );
