...
**File**: `app/api/email/pending-stream/route.ts`
...
export async function runCountNewEmailsJob() {
  ...
  try {
    // Get users with an email provider configured
...
3. EventSource connects to /api/email/pending-stream
...
```
1. Extract all emails since lastEmailSync
...
4. See `/api/email/pending-stream`
